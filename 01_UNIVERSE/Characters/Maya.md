# Maya — Character Bible

**Character ID:** CHAR-02
**Reference ID:** MAYA-REF-001
**Version:** v001
**Status:** CANONICAL / LOCKED

## Role

Best friend / intelligent creative problem-solver

## Age / Character Scale

4–5 years old

## Personality

- Intelligent
- Creative
- Thoughtful
- Curious
- Confident
- Kind
- Supportive
- Observant
- Encouraging
- Playful

Maya naturally thinks before acting and enjoys discovering how things work. She helps her friends solve problems through creativity and simple, age-appropriate reasoning.

Her intelligence should feel natural for a 4–5-year-old child, never like an adult speaking through a child.

## Story Function

Maya contributes ideas, notices details others may miss, and helps the group find simple and creative solutions.

She is the group's thoughtful problem-solver and often turns confusing situations into understandable ideas.

## Visual Bible

**Status:** CANONICAL / LOCKED

### Locked Visual DNA

- Age appearance: 4–5
- Skin: Warm medium brown
- Eyes: Large warm brown
- Face: Rounded preschool proportions
- Hair: Dark brown
- Hairstyle: Two rounded hair buns — LOCKED
- Front hair/bangs: LOCKED
- Headband: Purple — LOCKED
- Star hair clip: One small yellow five-point star
- Star clip position: Maya's anatomical LEFT side — LOCKED
- Top: Lavender
- Top emblem: Warm yellow lightbulb
- Skirt: Purple with subtle light lavender polka dots
- Shoes: White with purple accents
- Body style: Rounded premium preschool 3D
- Overall silhouette: LOCKED

### Character Consistency Rules

1. Maya must remain the same physical character in every generation.
2. The two-bun hairstyle must never be redesigned.
3. Bun size, placement, and silhouette must remain consistent.
4. The purple headband must remain present and correctly positioned.
5. The yellow star hair clip must remain on Maya's anatomical LEFT side.
6. The star clip must never be duplicated or mirrored.
7. Facial proportions must remain consistent.
8. Skin tone must remain consistent.
9. Eye color and general eye shape must remain consistent.
10. Maya must retain her 4–5-year-old appearance.
11. Body proportions must remain consistent.
12. The lavender top must remain lavender.
13. The yellow lightbulb emblem must remain on the front of the top.
14. The purple skirt and polka-dot pattern must remain consistent.
15. Shoes must remain white with purple accents.
16. Random accessories must not be introduced.
17. Maya must remain visually compatible with Leo and the rest of the Children Rhymes AI universe.
18. Camera angle, pose, expression, environment, lighting, and action may change.
19. Maya's core visual identity may not change without an explicit approved design revision.

## Canonical Camera Reference

Maya has been validated using the eight-angle production reference:

| Angle | View |
|---:|---|
| 0° | Front |
| 45° | Front-left |
| 90° | Left profile |
| 135° | Back-left |
| 180° | Back |
| 225° | Back-right |
| 270° | Right profile |
| 315° | Front-right |

The angle represents the camera position around Maya.

Maya's anatomical left and right must never be interpreted as viewer-left and viewer-right.

## Reference Hierarchy

1. MAYA-REF-001 canonical reference images
2. Locked Visual DNA
3. Character personality
4. Scene-specific action
5. Environment
6. Camera
7. Lighting

Scene-specific elements may change.

Maya's identity may not.

## Signature Accessory

Maya's purple idea/story notebook is a scene-dependent prop, not a permanent part of her physical character design.

It may be used when the story requires it.

## Voice

**Status:** Not yet locked.

Voice characteristics will be finalized separately from the visual character design.

## Canonical References

The following references are canonical:

- `References/MAYA-REF-001-FullBody.jpeg`
- `References/MAYA-REF-001-Portrait.jpeg`
- `References/MAYA-REF-001-Turnaround-8Angle.jpeg`

## Revision Policy

MAYA-REF-001 is locked.

Any future change to Maya's:

- face
- hair
- hairstyle
- headband
- star clip
- clothing
- colors
- body proportions
- shoes
- overall silhouette

requires an explicit character-design revision and a new reference version.

Do not silently modify the canonical design.
